    ...
    def post(self, request):
        ...
        try:
            ...
            if not all([username, email, password]):
                logger.warning(
                    f"Registration attempt with missing fields - username: {bool(username)}, email: {bool(email)}, password: {bool(password)}"
                )
                return Response(
                    {"detail": "Please provide username, email and password"}, status=status.HTTP_400_BAD_REQUEST
                )
