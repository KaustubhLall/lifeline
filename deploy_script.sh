...
# Certbot creates a server block for port 443, but we want to define our own.
# We will remove the server block generated by certbot from the configuration file.
sudo sed -i '/server {/,$d' /etc/nginx/conf.d/lifeline.conf

sudo tee -a /etc/nginx/conf.d/lifeline.conf > /dev/null <<EOT
server {
